...
Ogre::Camera* gkEngine::getCamera() {

	Ogre::RenderSystem *rs 	= m_private->root->getRenderSystemByName("OpenGL ES 2.x Rendering Subsystem");
	Ogre::RenderTarget *rt 	= rs->getRenderTarget("airgames"); // this happens to be the same name as the window title set in Main.cpp
	Ogre::Viewport *vp 		= rt->getViewport(0);
	
	//vp->setDimensions(0, 0, 0.5, 0.5);
	
	Ogre::ColourValue *col = new Ogre::ColourValue(1,0,0,0); 
	vp->setBackgroundColour(*col);
	//vp->setClearEveryFrame(true);
	vp->setOverlaysEnabled(false);
	//vp->setSkiesEnabled(false);
	
	//vp->setOrientationMode((Ogre::OrientationMode) 0);
	
	//Ogre::OrientationMode m = vp->getOrientationMode();
	
	/*
	switch (vp->getOrientationMode()) {
		
		case 0: gkLogMessage("@@@ 0\n"); break;
		case 1: gkLogMessage("@@@ 1\n"); break;
		case 2: gkLogMessage("@@@ 2\n"); break;
		case 3: gkLogMessage("@@@ 3\n"); break;
		default: gkLogMessage("@@@ ?\n"); break;
	}
	*/
	...
	*/
	...
	/*
	char str[100];
	sprintf(str, "Actual left: %d", vp->getActualLeft());
	gkLogMessage(str);
	
	sprintf(str, "Actual top: %d", vp->getActualTop());
	gkLogMessage(str);
	
	sprintf(str, "Actual width: %d", vp->getActualWidth());
	gkLogMessage(str);
	
	sprintf(str, "Actual height: %d", vp->getActualHeight());
	gkLogMessage(str);
	*/
